...
export default function ApplyWizard({ route, navigation }) {
  ...
  // --- ✨ 1. Fetch Job Config AND Profile Documents ---
  useEffect(() => {
    (async () => {
      try {
        ...
        // Profile documents fetch logic
        let profileDocs = {};
        if (userId) {
          const userSnap = await getDoc(doc(db, "users", userId));
          if (userSnap.exists()) {
            profileDocs = userSnap.data().profileDocuments || {};
          }
        }
        ...
        if (formSnap.exists()) {
          ...
          // --- ✨ AUTO-FILL: Match Required Docs with Profile ---
          const preFilled = {};
          if (formData.documents) {
            formData.documents.forEach(docReq => {
              if (profileDocs[docReq.name]) {
                preFilled[docReq.name] = profileDocs[docReq.name]; // Link match hua
              }
            });
          }
          ...
          <View>
            ...
            {(formConfig.formFields || []).map(f => (
              <View key={f.id} style={{marginBottom: 24}}>
                ...
                ) : ( <FloatingInput label={`${f.title} ${f.required ? '*' : ''}`} value={responses[f.id]?.toString() || ''} onChangeText={t => setResponses({...responses, [f.id]: t})} /> )}
              </View>
            ))}
